...
public:
    explicit LoadPartitioning(int method, uint64_t tasks, uint64_t chunk,
    ...
    uint64_t getNextChunk() {
        ...
        switch (schedulingMethod) {
            ...
            case FISS: { //fixed increase self-scheduling (FISS)
                ...
                chunkSize = initChunk + schedulingStep *
                    (uint64_t)ceil((2.0 * totalTasks * (1.0 - (fissStages / X)))
                            / (totalWorkers * fissStages * (fissStages - 1))); //chunksize with increment after init
                break;
            }
